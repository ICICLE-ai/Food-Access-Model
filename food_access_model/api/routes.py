...
from food_access_model.api.models import DrawableAgent, GetDrawableAgentsRequest, RectangleBound, parse_drawable_agents_request, MIN_ZOOM_FOR_POLYGONS, MAX_RENDERING_POINTS, DrawableAgentEncoder, MIN_ZOOM_FOR_SUPERCLUSTERS
...
from food_access_model.model_multi_processing.superclusters import process_superclusters
from food_access_model.model_multi_processing.filtering import filter_by_bounds_parallel
from food_access_model.model_multi_processing.sse_superclusters import stream_superclusters
from food_access_model.model_multi_processing.sse_filtering import stream_filter_by_bounds
...
@router.get("/drawable-agents-sse")
async def get_drawable_agents_sse(
    request: GetDrawableAgentsRequest = Depends(parse_drawable_agents_request),
    repository: DBRepository = Depends(get_db_repository)
):
    async def stream_drawable_agents():
        agents_list = repository.get_model().space.agents
        should_use_supercluster = (
            len(agents_list) > MAX_RENDERING_POINTS
            and request.zoom < MIN_ZOOM_FOR_SUPERCLUSTERS
        )

        logger.info(f"Should use supercluster: {should_use_supercluster}")
        if should_use_supercluster:
            async for item in stream_superclusters(request, repository):
                dump = json.dumps(item, cls=DrawableAgentEncoder)
                yield f"data:{dump}\n\n"
        else:
            count = 0
            async for agent in stream_filter_by_bounds(agents_list, request.bounds):
                dump = json.dumps(agent, cls=DrawableAgentEncoder)
                yield f"data:{dump}\n\n"

    return StreamingResponse(stream_drawable_agents(), media_type="text/event-stream")

@router.get("/drawable-agents", response_model=list[DrawableAgent])
def get_drawable_agents(
    request: GetDrawableAgentsRequest = Depends(parse_drawable_agents_request),
    repository: DBRepository = Depends(get_db_repository),
):
    logger.info(f"Getting drawable agents for request: {request}")
    ...
    should_use_supercluster = (
        len(agents_list) > MAX_RENDERING_POINTS
        and request.zoom < MIN_ZOOM_FOR_POLYGONS
    )

    logger.debug(f"Should use supercluster: {should_use_supercluster}")
    if should_use_supercluster:
        return process_superclusters(request, repository)
    
    

    logger.debug(f"Agents list length: {len(agents_list)}")
    filtered_agents_list = filter_by_bounds_parallel(agents_list, request.bounds, request.max_elements, request.zoom)
    logger.info(f"Filtered agents {len(filtered_agents_list)} by bounds")
    
    if len(filtered_agents_list) == 0:
        logger.info(f"No agents found within bounds")
        return []
...
async def remove_store(
    store_name: str = Body(...), repository: DBRepository = Depends(get_db_repository)
):
    model = repository.get_model()
...
async def add_store(
    store: StoreInput, repository: DBRepository = Depends(get_db_repository)
):
    model = repository.get_model()
    store_data = {
        "name": store.name,
