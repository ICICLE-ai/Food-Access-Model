...
def process_agent_for_rendering(
    agent: DrawableAgent, should_show_polygons: bool
) -> DrawableAgent:
...
def preprocess_agents_for_rendering(agents) -> list[DrawableAgent]:

    logger.debug(f"Preprocessing agents for rendering")
    logger.debug(f"Agents list length: {len(agents)}")
    should_show_polygons = len(agents) < MAX_RENDERING_POINTS
...
def serial_filter_by_bounds(agents, rectangle_bound: RectangleBound, should_show_polygons: bool) -> List:
    """
    ...
    return [process_agent_for_rendering(agent, should_show_polygons) for agent in agents if bbox.contains(agent.centroid)]


async def stream_filter_by_bounds(
    ...
    if rectangle_bound is None:
        ...
        for agent in agents[:max_elements]:
            yield agent
        return
    ...
    if len(agents) < max_elements:
        ...
        for agent in serial_filter_by_bounds(agents, rectangle_bound, should_show_polygons):
            yield agent
        return
    ...
    if len(batches) > 1:
        ...
        try:
                ...
                for future in concurrent.futures.as_completed(
                    ...
                    try:
                        ...
                        if result_ids is not None:
                            # filtered_agents_ids.extend(result)
                            for idx in result_ids:
                                current_index += 1
                                if current_index > max_elements:
                                    return
                                yield preprocess_agent_for_rendering(agents[idx], should_show_polygons)
                                yield agents[idx]
                        ...
                        result_ids = _filter_agents_batch(batch_info)
                        for idx in result_ids:
                            current_index += 1
                            if current_index > max_elements:
                                return
            ...
            for agent in candidates:
                if bbox.contains(agent.centroid):
                    yield preprocess_agent_for_rendering(agent, should_show_polygons)
    else:
        logger.info(
    ...
    # logger.info(f"Filtered to {len(filtered_agents_ids)} agents from {len(agents)} original agents")

    # filtered_agents = [agent for agent in agents if agent.id in filtered_agents_ids]
